...
export function RiskBadgeInline({ riskLevel, justification, compact = false }: RiskBadgeInlineProps) {
  ...
  if (compact) {
    return (
      <span className={cn('inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full', cfg.color)}>
  ...
  return (
    <div className={cn('flex items-start gap-3 p-4 rounded-lg border', cfg.color.replace('text-', 'border-').replace('bg-', ''))}>
